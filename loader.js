...
document.addEventListener('DOMContentLoaded', () => {
  const subtitleMeta = document.querySelector('meta[name="post-subtitle"]');
  const imageMeta = document.querySelector('meta[name="post-image"]');
  if (!subtitleMeta || !imageMeta) return;

  const description = subtitleMeta.content.trim();
  const imageURL = imageMeta.content.trim();
  const pageTitle = document.title;
  const pageURL = window.location.href.replace(/^http:\/\//i, 'https://');

  function createOrUpdateMeta(property, content, isName = false) {
  ...
  createOrUpdateMeta('twitter:image', imageURL, true);
