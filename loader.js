document.addEventListener("DOMContentLoaded", () => {
  if (localStorage.getItem("supportWarningAccepted") !== "true") {
    ...
    modal.innerHTML = `
      <div style="margin-bottom: 20px;"><i class="fa-solid fa-triangle-exclamation" style="font-size: 50px;"></i></div>
      <h2 style="font-size: 1.8rem;">¡Hay mudanzas!</h2>
      <p style="opacity: 0.8; font-size: 1.1rem; line-height: 1.6; margin-bottom: 25px;">
        Nos hemos mudado a <a href="https://ivandfx.gitlab.io" style="color: var(--link-hover);">GitLab</a>.<br>
        La web en GitHub seguirá disponible por tiempo limitado y con menos funciones.</p>
      <div style="display: flex; justify-content: center;">
        <button id="close-warning-dfx" class="pb-button" style="cursor: pointer; border: none; font-family: 'Gabarito'; font-size: 1rem; width: 180px;">
          Entendido
        </button>
      </div>
    `;
    ...
    document.getElementById("close-warning-dfx").onclick = () => {
      overlay.remove();
    };
  }
});
...
const currentURL = encodeURIComponent(
  window.location.href.replace(/^http:\/\//i, ''),
...
function handleWhatsappShare(event) {
  event.preventDefault();
  const whatsappUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(getUnifiedShareText())}`;
